...
void assemble_sdt_to(tokenised_line *tokenised_line, int line, char *binary_string) {
  ...
  if (!(strcmp(opcode, "ldr"))) {
    ...
    if (expression[0] == '=') {
      ...
      // less than or equal to 256 because 8 bits
...
//  if (expression[1] == '[') {
//    char delim[15] = " []r";
//    // if just rn no offset
//    char *token = strtok(expression, delim);
//    set_rn = (int) strtol(token, NULL, 10);
//    // if rn plus offset
//    snprintf(delim, sizeof(delim), "%s%s" , delim, token);
//    char *offset = token = strtok(expression, delim);
//    set_offset = (int) strtol(offset, NULL, 10);
//  }
